...
def _get_metrics_and_analysis():
    """Shared helper: fetch metrics and run analysis."""
...
# ─────────────────────────────────────────────
# GET /metrics  →  Raw metrics from Prometheus
# ─────────────────────────────────────────────
@app.route("/metrics", methods=["GET"])
def get_metrics():
    try:
        metrics = fetch_all_metrics()
        # Strip non-serializable trend lists for clean JSON output
        ...
        return jsonify({"status": "ok", "timestamp": datetime.utcnow().isoformat(), "metrics": serializable})
...
# ─────────────────────────────────────────────
# GET /analysis  →  Analyzed issues JSON
# ─────────────────────────────────────────────
...
# ─────────────────────────────────────────────
# GET /report/pdf  →  Downloadable PDF report
# ─────────────────────────────────────────────
...
# ─────────────────────────────────────────────
# POST /send-report  →  EMS Integration endpoint
# ─────────────────────────────────────────────
@app.route("/send-report", methods=["POST"])
def send_report():
    """
    EMS Integration: Generate PDF and return as downloadable response.
    Can be extended to email/upload the report to EMS.
    """
    try:
        metrics, analysis = _get_metrics_and_analysis()
        pdf_bytes = generate_pdf_report(metrics, analysis)
...
def health():
    return jsonify({"status": "ok", "service": "EMS Monitoring API", "timestamp": datetime.utcnow().isoformat()})
